 ...
 * gui_state (step 6a -- written by gui_view_canvas's declare_canvas_strip, read by declare_tooltips
 * below). The right settings panel's disclosure/dropdown-open bits and numeric-field edit buffers moved to
 * gui_view_settings.c (step 4 -- panel-local). GUI_MAX_TARGETS and k_playback_names live in
 * gui_defs.h (shared with declare_export_modal here / declare_canvas_preview in gui_view_canvas.c). */
 ...
 * decomposition step 5). declare_row_tooltips stays here (chrome-owned per the plan's §3 fan-out
 * table; moves to gui_view_chrome in step 6b). */
