...
static tp_status reader_read_all_bounded(tp_journal_io *io, uint8_t **out,
    ...
    *out_len = 0U;
    ...
    if ((uint64_t)advertised > (uint64_t)tp_journal__file_limit()) {
        return tp_error_set(err, TP_STATUS_OUT_OF_BOUNDS,
                            "journal exceeds the supported byte bound");
    }
    const size_t limit = tp_journal__file_limit();
...
static tp_status walk_records(const uint8_t *buf, size_t len, tp_idset *ids, tp_jrn_meta_ref *meta,
    ...
    *stop = TP_JRN_STOP_EOF;
    ...
    while (off < len) {
        ...
        if (*frame_count >= tp_journal__record_limit()) {
            *stop_off = off;
            return TP_STATUS_OUT_OF_BOUNDS;
        }
...
tp_status tp_journal_recover(tp_journal *j, tp_journal_recovery *out, tp_error *err) {
    ...
    uint8_t *buf = NULL;
    size_t len = 0;
    tp_status read_status = reader_read_all_bounded(&j->io, &buf, &len, err);
...
tp_status tp_journal_peek(tp_journal_io io, tp_journal_peek_result *out, tp_error *err) {
    ...
    uint8_t *buf = NULL;
    size_t len = 0;
    tp_status read_status = reader_read_all_bounded(&io, &buf, &len, err);
    if (io.destroy) {
