...
tp_format_owned_descriptor *tp_format_owned_descriptor_clone(
    ...
    if (!descriptor || !descriptor->id || !descriptor->display_name ||
        descriptor->artifact_count < 0 ||
        descriptor->artifact_count > (int)TP_FORMAT_OUTPUT_MAX ||
        (descriptor->artifact_count > 0 && !descriptor->artifacts) ||
        descriptor->host_fact_count < 0 ||
        descriptor->host_fact_count > (int)TP_FORMAT_HOST_FACT_MAX ||
        (descriptor->host_fact_count > 0 && !descriptor->host_facts)) {
        tp_error_set(error, TP_STATUS_INVALID_ARGUMENT,
                     "invalid format descriptor clone source");
        return NULL;
    }
    for (int i = 0; i < descriptor->artifact_count; ++i) {
        if (!descriptor->artifacts[i].id ||
            !descriptor->artifacts[i].suffix) {
            tp_error_set(error, TP_STATUS_INVALID_ARGUMENT,
                         "invalid format artifact clone source");
            return NULL;
        }
    }
    for (int i = 0; i < descriptor->host_fact_count; ++i) {
        if (!descriptor->host_facts[i].id ||
            !descriptor->host_facts[i].output_id ||
            !descriptor->host_facts[i].root_marker) {
            tp_error_set(error, TP_STATUS_INVALID_ARGUMENT,
                         "invalid format host-fact clone source");
            return NULL;
        }
    }
    tp_format_owned_descriptor *owned =
    ...
    if (!owned) {
        tp_error_set(error, TP_STATUS_OOM, "format descriptor clone OOM");
        return NULL;
    }
    owned->view = *descriptor;
    owned->view.id = format_strdup(descriptor->id);
    owned->view.display_name = format_strdup(descriptor->display_name);
    owned->view.artifacts = owned->artifacts;
    owned->view.host_facts = descriptor->host_fact_count > 0
                                 ? owned->host_facts
                                 : NULL;
    ...
    for (int i = 0; i < descriptor->artifact_count; ++i) {
        owned->artifacts[i] = descriptor->artifacts[i];
        owned->artifacts[i].id = format_strdup(descriptor->artifacts[i].id);
        owned->artifacts[i].suffix =
            format_strdup(descriptor->artifacts[i].suffix);
        if (!owned->artifacts[i].id || !owned->artifacts[i].suffix) {
            tp_format_owned_descriptor_destroy(owned);
            tp_error_set(error, TP_STATUS_OOM, "format descriptor clone OOM");
            return NULL;
    ...
    for (int i = 0; i < descriptor->host_fact_count; ++i) {
        owned->host_facts[i] = descriptor->host_facts[i];
        owned->host_facts[i].id =
            format_strdup(descriptor->host_facts[i].id);
        owned->host_facts[i].output_id =
            format_strdup(descriptor->host_facts[i].output_id);
        owned->host_facts[i].root_marker =
            format_strdup(descriptor->host_facts[i].root_marker);
        if (!owned->host_facts[i].id || !owned->host_facts[i].output_id ||
            !owned->host_facts[i].root_marker) {
            tp_format_owned_descriptor_destroy(owned);
            tp_error_set(error, TP_STATUS_OOM, "format descriptor clone OOM");
            return NULL;
