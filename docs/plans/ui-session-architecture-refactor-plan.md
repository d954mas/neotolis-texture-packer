...
| §4 R2d closed lifecycle ownership; history/identity/source commands stayed wherever a session was borrowed | Undo/Redo/Save/SaveAs/invalidate and their capability/depth queries re-homed onto the host owner (`gui_host_binding`), enforced by a new `R2d single host command owner` checker sweep | `d914b50` |
