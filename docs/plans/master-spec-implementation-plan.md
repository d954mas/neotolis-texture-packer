...
- **P1-1** — undo/redo не аппендят в журнал (`tp_history.c`), а `tp_model_recover` читает хвост → краш на позиции undo воскрешает отменённую правку. В diff-журнале undo/redo аппендят обратную операцию через тот же append-гейт. Severity — wrong-result (файл проекта не трогается; восстановленное всегда dirty за Save-промптом), окно узкое (краш ровно после undo без последующего коммита).
