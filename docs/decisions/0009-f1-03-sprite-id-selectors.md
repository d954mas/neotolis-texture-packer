...
2. **Кто триггерит persist**: миграция pending→v4 применяется + персистится, только когда
   какой-то WRITABLE поток делает resolution-скан И save. Сейчас это делает GUI live/save;
   ordinary mutating CLI НЕ сканирует, так что v3 файл, отредактированный только через
   `ntpacker set/sprite/...`, сохраняется с pending `{name}` записями (валидное v4 состояние,
   версия уже 4). Явный перенос в CLI (op-engine/`migrate` verb) — задел F2. Механизм
   корректен и не сканирует на load; выбор точки вызова — открытый.
